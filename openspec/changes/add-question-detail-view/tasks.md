...
- [ ] 1.1 Add `get_question_detail(db, question_id)` service in `backend/app/services/sm2.py`
...
- [ ] 2.1 Add `GET /questions/{question_id}` endpoint in `backend/app/routers/questions.py`
...
- [ ] 3.1 Add API e2e tests for `GET /questions/{id}` under `e2e/api/`
...
- [ ] 4.1 Add `QuestionDetail` interface and `getQuestion(id)` function to `frontend/lib/api.ts`
...
- [ ] 5.1 Create `frontend/app/questions/[id]/page.tsx` skeleton with header, loading state, fetch effect
  ...
  - Depends on: 4.1
  - Independence: serial
- [ ] 5.2 Render four content sections (question text with category/difficulty/tags chips, reference answer, key_points, common_mistakes)
  ...
  - Depends on: 5.1
  - Independence: serial
- [ ] 5.3 Add bottom button row with "返回選題" and "開始練習這題"
  ...
  - Depends on: 5.1
  - Independence: serial
- [ ] 5.4 Handle fetch error states (network, 404, other 4xx/5xx)
...
- [ ] 6.1 Add a secondary "查看內容" button beside the "選擇練習" CTA on each card in `frontend/app/questions/select/page.tsx`
...
- [ ] 7.1 Add Playwright e2e cases for the detail page flow under `e2e/playwright/`
